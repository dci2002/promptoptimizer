...
- [2026-09-24 19:54:00] Cooperative stop: a shared `threading.Event` is threaded through `run_optimization` → `ReActAgent` as `stop_event`; the agent checks it via `_check_stop()` (raises `StopRequested`) between iterations and before each blocking LLM call inside tools, so a stop is honored within one LLM call at most. `StopRequested` is caught in `run_optimization` and converted to a failed `OptimizationResult` ("stopped by user") — it never escapes to the worker's generic exception handler.
- [2026-09-24 19:54:00] `get_state()` response shape now includes `stop_button: {enabled: bool}` (enabled while a run is in progress) — JS drives the Stop button's disabled state from it via the 500 ms poll.
